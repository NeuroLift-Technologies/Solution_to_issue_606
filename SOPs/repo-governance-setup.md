...
1. Repo-local governance (private, primary):
   https://github.com/NeuroLift-Technologies/Solution_to_issue_606/blob/main/NLT-DEV-OTOI.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/Solution_to_issue_606/blob/main/governance/NLT-DEV-OTOI.md
2. Internal gateway (private, primary):
   https://github.com/NeuroLift-Technologies/Solution_to_issue_606/blob/main/AGENTS.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/Solution_to_issue_606/blob/main/governance/AGENTS.md
...
Format reference: `NeuroLift-Technologies/Solution_to_issue_606` templates directory.
```
...
Format reference: `templates/escalation.md` in `NeuroLift-Technologies/Solution_to_issue_606`.
```

### Step 5: Grant GitHub App Access to `Solution_to_issue_606`

Coding agents (e.g., GitHub Copilot, Codex) must be able to read the governance files in
`Solution_to_issue_606` from within the new repository. If the GitHub App installation is scoped
to **"Selected repositories"**, you must add `Solution_to_issue_606` to its access list.
...
3. Under **Repository access → Selected repositories**, add **`Solution_to_issue_606`**.
4. Click **Save**.
...
> in `Solution_to_issue_606` for full instructions.
...
- [ ] GitHub App has access to `Solution_to_issue_606` (or public mirror fallback is in place)

---

## Validation

Run the org-level validation script to check `Solution_to_issue_606` is healthy:

```bash
git clone https://github.com/NeuroLift-Technologies/Solution_to_issue_606
cd Solution_to_issue_606
